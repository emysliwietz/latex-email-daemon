...
def html_to_latex(html_content: str) -> str:
    ...
    def process_element(element, parent_tag=None):
        ...
        if isinstance(element, NavigableString):
            ...
            # DON'T strip() here or we lose spaces around formatted text!
            text = str(element)
            # Only skip if completely empty (but preserve whitespace-only text like " ")
    ...
    result = re.sub(r'\\\\(\\\\)+', r'\n\n', result)  # 2+ line breaks -> paragraph break
